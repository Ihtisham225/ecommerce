...
<div id="certificates-subtab-content" class="subtab-panel hidden">
    <div class="bg-gray-50 p-5 rounded-lg shadow-sm dark:bg-gray-700">
        <h3 class="text-lg font-medium text-gray-800 mb-4 dark:text-gray-200 flex items-center">
            ...
            {{ __('Course Certificates') }}
        </h3>

        {{-- Current Certificates --}}
        @if(isset($course) && $course->certificates)
            <div class="mb-6 p-4 border border-gray-200 rounded-lg bg-white dark:bg-gray-600 dark:border-gray-500 relative">
                <label class="block text-sm font-medium text-gray-700 mb-2 dark:text-gray-300">
                    {{ __('Current Certificates') }}
                </label>

                <!-- Remove Button -->
                <button type="button"
                        onclick="
                            document.getElementById('remove_certificates_flag').value = 1; 
                            this.closest('.mb-6').classList.add('hidden'); 
                            document.querySelectorAll('input[name=certificates_document_id]').forEach(el => el.checked = false);
                        "
                        class="absolute top-2 right-2 z-10 p-1 rounded-full bg-red-100 hover:bg-red-200 dark:bg-red-800 dark:hover:bg-red-700"
                        title="{{ __('Remove these certificates') }}">
                ...
                <div class="flex items-center space-x-4 p-3 border rounded-lg bg-blue-50 dark:bg-blue-900/20">
                    <span class="font-medium truncate dark:text-white">
                        {{ $course->certificates->name }}
                    </span>
                </div>
            </div>

            <!-- hidden flag -->
            <input type="hidden" name="remove_certificates" id="remove_certificates_flag" value="0">
        @endif
        ...
        <div class="mb-6">
            <label class="block text-sm font-medium text-gray-700 mb-2 dark:text-gray-300">
                {{ __('Select Certificate from Existing Documents') }}
            </label>

            <!-- Search Bar -->
            <div class="mb-3">
                <input type="text" id="certificates-search"
            ...
            <div id="certificates-list"
                ...
                @forelse($documents as $document)
                    @php
                        $isCertificate = $document->document_type === 'certificates';
                    @endphp

                    @if($isCertificate)
                        <label
                            class="certificates-item flex items-center space-x-3 p-3 border rounded-lg cursor-pointer transition-all hover:bg-blue-50 hover:border-blue-200 dark:hover:bg-blue-900/20 dark:border-gray-500"
                            data-name="{{ strtolower($document->name) }}">
                            
                            <input type="radio" name="certificates_id" value="{{ $document->id }}"
                                {{ old('certificates_id', $course->certificates->id ?? null) == $document->id ? 'checked' : '' }}
            ...
            <p class="text-xs text-gray-500 mt-2 dark:text-gray-400">
                {{ __('Choose one document to use as Course Certificate') }}
            </p>
        </div>

        <!-- Certificate Upload -->
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-2 dark:text-gray-300">{{ __('Upload New Certificate') }}</label>

            <div class="flex items-center justify-center w-full">
                <label for="dropzone-certificates"
                    ...
                    <div class="flex flex-col items-center justify-center pt-5 pb-6">
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400">{{ __('ZIP') }}</p>
                    </div>
                    <input id="dropzone-certificates" type="file" name="new_certificates" class="hidden"
                        accept=".zip" />
                </label>
            </div>

            <div id="certificates-preview" class="mt-4 hidden">
                ...
                <div class="flex items-center p-3 mt-2 border rounded-lg bg-green-50 dark:bg-green-900/20">
                    ...
                    <span id="certificates-file-name" class="text-sm truncate dark:text-white"></span>
                    <button type="button"
                        class="ml-auto text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300"
                        onclick="removeFile('dropzone-certificates', 'certificates-preview')">
            ...
            <p class="text-xs text-gray-500 mt-2 dark:text-gray-400">{{ __('Upload and set as Course Certificate') }}</p>
        </div>
    </div>
</div>
...
<script>
    document.getElementById('certificates-search').addEventListener('input', function () {
        ...
        let items = document.querySelectorAll('#certificates-list .certificates-item');
