<!-- Complete Document Sub-Tab -->
<div id="complete-document-subtab-content" class="subtab-panel hidden">
    <div class="bg-gray-50 p-5 rounded-lg shadow-sm dark:bg-gray-700">
        <h3 class="text-lg font-medium text-gray-800 mb-4 dark:text-gray-200 flex items-center">
            ...
            {{ __('Course Complete Document') }}
        </h3>

        {{-- Current Complete Document --}}
        @if(isset($course) && $course->completeDocument)
            <div class="mb-6 p-4 border border-gray-200 rounded-lg bg-white dark:bg-gray-600 dark:border-gray-500 relative">
                <label class="block text-sm font-medium text-gray-700 mb-2 dark:text-gray-300">
                    {{ __('Current Complete Document') }}
                </label>

                <!-- Remove Button -->
                <button type="button"
                        onclick="
                            document.getElementById('remove_complete_document_flag').value = 1; 
                            this.closest('.mb-6').classList.add('hidden'); 
                            document.querySelectorAll('input[name=complete_document_document_id]').forEach(el => el.checked = false);
                        "
                        class="absolute top-2 right-2 z-10 p-1 rounded-full bg-red-100 hover:bg-red-200 dark:bg-red-800 dark:hover:bg-red-700"
                        title="{{ __('Remove this complete document') }}">
                ...
                <div class="flex items-center space-x-4 p-3 border rounded-lg bg-blue-50 dark:bg-blue-900/20">
                    <span class="font-medium truncate dark:text-white">
                        {{ $course->completeDocument->name }}
                    </span>
                </div>
            </div>

            <!-- hidden flag -->
            <input type="hidden" name="remove_complete_document" id="remove_complete_document_flag" value="0">
        @endif
        ...
        <div class="mb-6">
            <label class="block text-sm font-medium text-gray-700 mb-2 dark:text-gray-300">
                {{ __('Select Complete Document from Existing Documents') }}
            </label>

            <!-- Search Bar -->
            <div class="mb-3">
                <input type="text" id="complete-document-search"
                    placeholder="{{ __('Search complete documents...') }}"
            ...
            <div id="complete-document-list"
                ...
                @forelse($documents as $document)
                    @php
                        $isCompleteDoc = $document->document_type === 'complete_document';
                    @endphp

                    @if($isCompleteDoc)
                        <label
                            class="complete-document-item flex items-center space-x-3 p-3 border rounded-lg cursor-pointer transition-all hover:bg-blue-50 hover:border-blue-200 dark:hover:bg-blue-900/20 dark:border-gray-500"
                            data-name="{{ strtolower($document->name) }}">
                            
                            <input type="radio" name="complete_document_id" value="{{ $document->id }}"
                                {{ old('complete_document_id', $course->completeDocument->id ?? null) == $document->id ? 'checked' : '' }}
                ...
                @empty
                    <p class="text-gray-500 col-span-3 py-4 text-center dark:text-gray-400">
                        {{ __('No complete documents available') }}
                    </p>
                @endforelse
            </div>

            <p class="text-xs text-gray-500 mt-2 dark:text-gray-400">
                {{ __('Choose one document to use as Course Complete Document') }}
            </p>
        </div>

        <!-- Complete Document Upload -->
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-2 dark:text-gray-300">{{ __('Upload New Complete Document') }}</label>

            <div class="flex items-center justify-center w-full">
                <label for="dropzone-complete-document"
                    ...
                    <input id="dropzone-complete-document" type="file" name="new_complete_document" class="hidden"
                        accept=".pdf,.doc,.docx,.jpg,.jpeg,.png,.webp" />
                </label>
            </div>

            <div id="complete-document-preview" class="mt-4 hidden">
                ...
                <div class="flex items-center p-3 mt-2 border rounded-lg bg-green-50 dark:bg-green-900/20">
                    ...
                    <span id="complete-document-file-name" class="text-sm truncate dark:text-white"></span>
                    <button type="button"
                        class="ml-auto text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300"
                        onclick="removeFile('dropzone-complete-document', 'complete-document-preview')">
            ...
            <p class="text-xs text-gray-500 mt-2 dark:text-gray-400">{{ __('Upload and set as Course Complete Document') }}</p>
        </div>
    </div>
</div>

<script>
    document.getElementById('complete-document-search').addEventListener('input', function () {
        let searchValue = this.value.toLowerCase();
        let items = document.querySelectorAll('#complete-document-list .complete-document-item');
