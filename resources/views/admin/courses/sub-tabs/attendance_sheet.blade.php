<!-- Attendance Sheet Sub-Tab -->
<div id="attendance-sheet-subtab-content" class="subtab-panel hidden">
    <div class="bg-gray-50 p-5 rounded-lg shadow-sm dark:bg-gray-700">
        <h3 class="text-lg font-medium text-gray-800 mb-4 dark:text-gray-200 flex items-center">
            ...
            {{ __('Course Attendance Sheet') }}
        </h3>

        {{-- Current Attendance Sheet --}}
        @if(isset($course) && $course->attendanceSheet)
            <div class="mb-6 p-4 border border-gray-200 rounded-lg bg-white dark:bg-gray-600 dark:border-gray-500 relative">
                <label class="block text-sm font-medium text-gray-700 mb-2 dark:text-gray-300">
                    {{ __('Current Attendance Sheet') }}
                </label>

                <!-- Remove Button -->
                <button type="button"
                        onclick="
                            document.getElementById('remove_attendance_sheet_flag').value = 1; 
                            this.closest('.mb-6').classList.add('hidden'); 
                            document.querySelectorAll('input[name=attendance_sheet_document_id]').forEach(el => el.checked = false);
                        "
                        class="absolute top-2 right-2 z-10 p-1 rounded-full bg-red-100 hover:bg-red-200 dark:bg-red-800 dark:hover:bg-red-700"
                        title="{{ __('Remove this attendance sheet') }}">
                ...
                <div class="flex items-center space-x-4 p-3 border rounded-lg bg-blue-50 dark:bg-blue-900/20">
                    <span class="font-medium truncate dark:text-white">
                        {{ $course->attendanceSheet->name }}
                    </span>
                </div>
            </div>

            <!-- hidden flag -->
            <input type="hidden" name="remove_attendance_sheet" id="remove_attendance_sheet_flag" value="0">
        @endif
        ...
        <div class="mb-6">
            <label class="block text-sm font-medium text-gray-700 mb-2 dark:text-gray-300">
                {{ __('Select Attendance Sheet from Existing Documents') }}
            </label>

            <!-- Search Bar -->
            <div class="mb-3">
                <input type="text" id="attendance-sheet-search"
                    placeholder="{{ __('Search attendance sheets...') }}"
            ...
            <div id="attendance-sheet-list"
                ...
                @forelse($documents as $document)
                    @php
                        $isAttendance = $document->document_type === 'attendance_sheet';
                    @endphp

                    @if($isAttendance)
                        <label
                            class="attendance-sheet-item flex items-center space-x-3 p-3 border rounded-lg cursor-pointer transition-all hover:bg-blue-50 hover:border-blue-200 dark:hover:bg-blue-900/20 dark:border-gray-500"
                            data-name="{{ strtolower($document->name) }}">
                            
                            <input type="radio" name="attendance_sheet_document_id" value="{{ $document->id }}"
                                {{ old('attendance_sheet_document_id', $course->attendanceSheet->id ?? null) == $document->id ? 'checked' : '' }}
                ...
                @empty
                    <p class="text-gray-500 col-span-3 py-4 text-center dark:text-gray-400">
                        {{ __('No attendance sheets available') }}
                    </p>
                @endforelse
            </div>

            <p class="text-xs text-gray-500 mt-2 dark:text-gray-400">
                {{ __('Choose one document to use as Attendance Sheet') }}
            </p>
        </div>

        <!-- Attendance Sheet Upload -->
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-2 dark:text-gray-300">{{ __('Upload New Attendance Sheet') }}</label>

            <div class="flex items-center justify-center w-full">
                <label for="dropzone-attendance-sheet"
                    ...
                    <input id="dropzone-attendance-sheet" type="file" name="new_attendance_sheet" class="hidden"
                        accept=".pdf,.doc,.docx,.jpg,.jpeg,.png,.webp" />
                </label>
            </div>

            <div id="attendance-sheet-preview" class="mt-4 hidden">
                ...
                <div class="flex items-center p-3 mt-2 border rounded-lg bg-green-50 dark:bg-green-900/20">
                    ...
                    <span id="attendance-sheet-file-name" class="text-sm truncate dark:text-white"></span>
                    <button type="button"
                        class="ml-auto text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300"
                        onclick="removeFile('dropzone-attendance-sheet', 'attendance-sheet-preview')">
            ...
            <p class="text-xs text-gray-500 mt-2 dark:text-gray-400">{{ __('Upload and set as Attendance Sheet') }}</p>
        </div>
    </div>
</div>

<script>
    document.getElementById('attendance-sheet-search').addEventListener('input', function () {
        let searchValue = this.value.toLowerCase();
        let items = document.querySelectorAll('#attendance-sheet-list .attendance-sheet-item');
