<!-- Invoice Sub-Tab -->
<div id="invoice-subtab-content" class="subtab-panel hidden">
    <div class="bg-gray-50 p-5 rounded-lg shadow-sm dark:bg-gray-700">
        ...
        {{-- Current Invoice --}}
        @if(isset($course) && $course->invoice)
            <div class="mb-6 p-4 border border-gray-200 rounded-lg bg-white dark:bg-gray-600 dark:border-gray-500 relative">
                ...
                <button type="button"
                        onclick="
                            document.getElementById('remove_invoice_flag').value = 1; 
                            this.closest('.mb-6').classList.add('hidden'); 
                            document.querySelectorAll('input[name=invoice_document_id]').forEach(el => el.checked = false);
                        "
                        class="absolute top-2 right-2 p-1 rounded-full bg-red-100 hover:bg-red-200 dark:bg-red-800 dark:hover:bg-red-700"
                        title="{{ __('Remove this invoice') }}">
            ...
            <!-- hidden flag -->
            <input type="hidden" name="remove_invoice" id="remove_invoice_flag" value="0">
        @endif

        <!-- Invoice Selection -->
        <div class="mb-6">
            <label class="block text-sm font-medium text-gray-700 mb-2 dark:text-gray-300">
                {{ __('Select Invoice from Existing Documents') }}
            </label>

            <!-- Search Bar -->
            <div class="mb-3">
                <input type="text" id="invoice-search"
                    placeholder="{{ __('Search invoice files...') }}"
            ...
            <div id="invoice-list"
                ...
                @forelse($documents as $document)
                    @php
                        ...
                        $isInvoice = $document->document_type === 'invoice';
                    @endphp

                    @if($isInvoice)
                        <label
                            class="invoice-item flex items-center space-x-3 p-3 border rounded-lg cursor-pointer transition-all hover:bg-blue-50 hover:border-blue-200 dark:hover:bg-blue-900/20 dark:border-gray-500"
                            data-name="{{ strtolower($document->name) }}">
                            
                            <input type="radio" name="invoice_document_id" value="{{ $document->id }}"
                                {{ old('invoice_document_id', $course->invoice->id ?? null) == $document->id ? 'checked' : '' }}
                ...
                @empty
                    <p class="text-gray-500 col-span-3 py-4 text-center dark:text-gray-400">
                        {{ __('No invoice documents available') }}
                    </p>
                @endforelse
            </div>

            <p class="text-xs text-gray-500 mt-2 dark:text-gray-400">
                {{ __('Choose one invoice document to use as Course Invoice') }}
            </p>
        </div>

        <!-- Invoice Upload -->
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-2 dark:text-gray-300">{{ __('Upload New Course Invoice') }}</label>

            <div class="flex items-center justify-center w-full">
                <label for="dropzone-invoice"
                    ...
                    <input id="dropzone-invoice" type="file" name="new_invoice" class="hidden"
                        accept=".pdf,.doc,.docx,.txt" />
                </label>
            </div>

            <div id="invoice-preview" class="mt-4 hidden">
                ...
                <div class="flex items-center p-3 mt-2 border rounded-lg bg-green-50 dark:bg-green-900/20">
                    ...
                    <span id="invoice-file-name" class="text-sm truncate dark:text-white"></span>
                    <button type="button"
                        class="ml-auto text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300"
                        onclick="removeFile('dropzone-invoice', 'invoice-preview')">
            ...
            <p class="text-xs text-gray-500 mt-2 dark:text-gray-400">{{ __('Upload and set as Course Invoice') }}</p>
        </div>
    </div>
</div>
...
<script>
    document.getElementById('invoice-search').addEventListener('input', function () {
        ...
        let items = document.querySelectorAll('#invoice-list .invoice-item');
