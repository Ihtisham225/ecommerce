<!-- Flyer Section for Schedule -->
<div id="flyer-subtab-content-{{ $index }}" class="flyer-subtab-content bg-gray-50 p-5 rounded-lg shadow-sm dark:bg-gray-700 mb-6">
    <h3 class="text-lg font-medium text-gray-800 mb-4 dark:text-gray-200 flex items-center">
        <svg class="w-5 h-5 mr-2 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"
            xmlns="http://www.w3.org/2000/svg">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M4 4h16v16H4z M4 8h16" />
        </svg>
        {{ __('Schedule Flyer') }}
    </h3>
    ...
    @if(!empty($flyer))
        <div class="mb-6 p-4 border border-gray-200 rounded-lg bg-white dark:bg-gray-600 dark:border-gray-500 relative">
            ...
            <button type="button"
                    onclick="
                        document.getElementById('remove_flyer_flag_{{ $index }}').value = 1;
                        this.closest('.mb-6').classList.add('hidden');
                        document.querySelectorAll('input[name=\'schedules[{{ $index }}][flyer_document_id]\']').forEach(el => el.checked = false);
                    "
                    class="absolute top-2 right-2 p-1 rounded-full bg-red-100 hover:bg-red-200 dark:bg-red-800 dark:hover:bg-red-700"
                    title="{{ __('Remove this flyer') }}">
            ...
            <div class="flex items-center space-x-4 p-3 border rounded-lg bg-blue-50 dark:bg-blue-900/20">
                <span class="font-medium truncate dark:text-white">
                    {{ $flyer['name'] ?? 'Flyer File' }}
                </span>
            </div>
        </div>

        <input type="hidden" name="schedules[{{ $index }}][remove_flyer]" id="remove_flyer_flag_{{ $index }}" value="0">
    @endif

    <!-- Flyer Selection -->
    <div class="mb-6">
        <label class="block text-sm font-medium text-gray-700 mb-2 dark:text-gray-300">
            {{ __('Select Flyer from Existing Documents') }}
        </label>

        <!-- Search Bar -->
        <div class="mb-3">
            <input type="text" id="flyer-search-{{ $index }}"
                placeholder="{{ __('Search flyer files...') }}"
        ...
        <div id="flyer-list-{{ $index }}"
            ...
            @forelse($documents as $document)
                @php
                    $isFlyer = $document->document_type === 'flyer';
                @endphp

                @if($isFlyer)
                    <label
                        class="flyer-item flex items-center space-x-3 p-3 border rounded-lg cursor-pointer transition-all hover:bg-blue-50 hover:border-blue-200 dark:hover:bg-blue-900/20 dark:border-gray-500"
                        data-name="{{ strtolower($document->name) }}">
                        
                        <input type="radio" 
                               name="schedules[{{ $index }}][flyer_document_id]" 
                               value="{{ $document->id }}"
                               {{ old("schedules.$index.flyer_document_id", $flyer['id'] ?? null) == $document->id ? 'checked' : '' }}
            ...
            @empty
                <p class="text-gray-500 col-span-3 py-4 text-center dark:text-gray-400">
                    {{ __('No flyer documents available') }}
                </p>
            @endforelse
        </div>

        <p class="text-xs text-gray-500 mt-2 dark:text-gray-400">
            {{ __('Choose one flyer document for this schedule') }}
        </p>
    </div>
    ...
    <div>
        ...
        <div class="flex items-center justify-center w-full">
            <label for="dropzone-flyer-{{ $index }}"
                ...
                <div class="flex flex-col items-center justify-center pt-5 pb-6">
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400">{{ __('PDF, PNG, JPG') }}</p>
                </div>
                <input id="dropzone-flyer-{{ $index }}" type="file" 
                       name="schedules[{{ $index }}][new_flyer]" 
                       class="hidden" accept=".pdf,.png,.jpg,.jpeg">
            </label>
        </div>

        <div id="flyer-preview-{{ $index }}" class="mt-4 hidden">
            ...
            <div class="flex items-center p-3 mt-2 border rounded-lg bg-green-50 dark:bg-green-900/20">
                ...
                <span id="flyer-file-name-{{ $index }}" class="text-sm truncate dark:text-white"></span>
                <button type="button"
                        class="ml-auto text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300"
                        onclick="removeFile('dropzone-flyer-{{ $index }}', 'flyer-preview-{{ $index }}')">
        ...
        <p class="text-xs text-gray-500 mt-2 dark:text-gray-400">
            {{ __('Upload and set as flyer for this schedule') }}
        </p>
    </div>
</div>

<script>
document.getElementById('flyer-search-{{ $index }}').addEventListener('input', function () {
    let searchValue = this.value.toLowerCase();
    let items = document.querySelectorAll('#flyer-list-{{ $index }} .flyer-item');
    items.forEach(item => {
