        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6 bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                <form method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div class="flex items-end gap-2">
                        <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700">{{ __('Filter') }}</button>
                        <a href="{{ route('admin.collections.index') }}" class="px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-600">{{ __('Reset') }}</a>
                    </div>
                </form>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-semibold">{{ __('Collections') }}</h3>
                    <a href="{{ route('admin.collections.create') }}" class="px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700">{{ __('Add Collection') }}</a>
                </div>
                        ...
                        <tbody>
                            @forelse($collections as $collection)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                    <td class="px-4 py-2 border">{{ $collection->title }}</td>
                                    <td class="px-4 py-2 border">{{ $collection->description ?? '-' }}</td>
                                    <td class="px-4 py-2 border">
                                        <span class="px-2 py-1 text-xs rounded-full {{ $collection->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="px-4 py-2 text-center">{{ __('No collections found.') }}</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
