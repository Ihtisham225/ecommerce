            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6">
                <div class="max-w-4xl mx-auto">
                    <div class="bg-indigo-600 px-6 py-4 rounded-t-lg">
                        <h1 class="text-2xl font-bold text-white">{{ __('Edit Collection') }}</h1>
                    </div>
                    ...
                    <div class="p-6 border border-gray-200 border-t-0 rounded-b-lg">
                        <form action="{{ route('admin.collections.update', $collection) }}" method="POST" class="space-y-6">
                            @csrf @method('PUT')
                            @include('admin.collections.partials.form', ['collection' => $collection])
                            ...
                            <div class="flex justify-between pt-4 border-t border-gray-200">
                                ...
                                <button type="submit" class="px-6 py-2 bg-indigo-600 text-white font-medium rounded-md hover:bg-indigo-700">{{ __('Update Collection') }}</button>
                            </div>
                        </form>
                    </div>
                </div>
