                ...
                <div class="px-6 py-6 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <p class="text-lg font-semibold">{{ $instructor->email }}</p>
                        </div>

                        <div>
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('Phone') }}</p>
                            <p class="text-lg font-semibold">{{ $instructor->phone ?? '-' }}</p>
                        </div>

                        <div>
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('Specialization') }}</p>
                            <p class="text-lg font-semibold">{{ $instructor->specialization ?? '-' }}</p>
                        </div>
                    ...
                    <!-- Bio -->
                    <div>
                        <h4 class="text-lg font-semibold mb-2 text-gray-700 dark:text-gray-300">{{ __('Bio') }}</h4>
                        <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg prose dark:prose-invert max-w-none">
                            {!! $instructor->bio ?? __('No bio available') !!}
                        </div>
                    </div>

                    <!-- CV -->
                    <div>
                        <h4 class="text-lg font-semibold mb-2 text-gray-700 dark:text-gray-300">{{ __('Curriculum Vitae (CV)') }}</h4>
                        @if($instructor->cv)
                            <div class="flex items-center justify-between bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
                                <span class="text-gray-700 dark:text-gray-300">{{ $instructor->cv->name }}</span>
                                <div class="space-x-2">
                                    <!-- Preview -->
                                    <a href="{{ route('admin.documents.show', $instructor->cv) }}"
                                       target="_blank"
                                       class="px-3 py-1 bg-indigo-600 hover:bg-indigo-700 text-white text-sm rounded-lg">
                                        {{ __('Preview') }}
                                    </a>
                                    <!-- Download -->
                                    <a href="{{ asset('storage/' . $instructor->cv->file_path) }}" 
                                       download
                        ...
                        @else
                            <p class="text-gray-500 dark:text-gray-400">{{ __('No CV uploaded') }}</p>
                        @endif
                    </div>
