        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6 bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <select name="country_id" class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded">
                            <option value="">{{ __('All') }}</option>
                    ...
                    <div>
                        ...
                        <input type="text" name="search" value="{{ request('search') }}"
                            placeholder="{{ __('Search by sponsor name...') }}"
                    ...
                    <div class="flex items-end">
                        ...
                        <a href="{{ route('admin.sponsors.index') }}" class="ml-2 px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-600">
                            {{ __('Reset') }}
                        </a>
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-semibold">{{ __('Sponsors') }}</h3>
                    <div class="flex justify-between items-center mb-4">
                    ...
                    <table class="min-w-full border">
                        <thead class="bg-gray-100 dark:bg-gray-700">
                            <tr>
                                ...
                                <th class="px-4 py-2 border">{{ __('Website') }}</th>
                                <th class="px-4 py-2 border">{{ __('Email') }}</th>
                                <th class="px-4 py-2 border">{{ __('Phone') }}</th>
                                <th class="px-4 py-2 border">{{ __('Country') }}</th>
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                    <td class="px-4 py-2 border">
                                        @if($sponsor->sponsorLogo)
                                            <img src="{{ asset('storage/' . $sponsor->sponsorLogo->file_path) }}" alt="{{ $sponsor->name }}" class="w-12 h-12 rounded-full">
                                        @else
                                            <span class="text-gray-500">{{ __('No Logo') }}</span>
                                        @endif
                                    </td>
                                    <td class="px-4 py-2 border">{{ $sponsor->name }}</td>
                                    <td class="px-4 py-2 border">
                                        @if($sponsor->website)
                                    ...
                                    <td class="px-4 py-2 border">{{ $sponsor->contact_email ?? '-' }}</td>
                                    <td class="px-4 py-2 border">{{ $sponsor->contact_phone ?? '-' }}</td>
                                    <td class="px-4 py-2 border">{{ $sponsor->country->name ?? '-' }}</td>
                                    <td class="px-4 py-2 border">
                                        <span class="px-2 py-1 text-xs rounded-full {{ $sponsor->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
